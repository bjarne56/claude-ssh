...
- 2026-05-04 skill 名 ssh-ops → sshops (slash 命令 `/sshops`), commit `ea213bc`+`60fbccc`+`7000449`:SKILL.md frontmatter `name` 改 sshops, install.sh 创建实体目录 `~/.claude/skills/sshops/` 替代旧 ssh-ops symlink-到-整仓库;SKILL.md 主源改英文 (128 行精简), 31 种语言只翻译 frontmatter description, 全部集中在 `skill-locales/descriptions.json` (单一来源, 加新语言只 JSON 加 1 行 KV);install.sh 按系统 locale 自动选 description (macOS AppleLocale / LANG, alias 规一 + 前缀 fallback + en 终极 fallback), 用 python 一行替换 description 行
