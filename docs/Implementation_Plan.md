...
- 2026-05-04 skill 名 ssh-ops → sshops + 31 国 description i18n: SKILL.md frontmatter `name` 改 sshops (slash 命令 `/sshops`); install.sh 删旧 symlink 改建实体目录 ~/.claude/skills/sshops/ (SKILL.md cp 自项目根英文 master, 业务文件 symlink); 31 种语言 description 集中到 skill-locales/descriptions.json (单 JSON, 之前 32 个独立 SKILL.<lang>.md 删除净 -3943 行); install.sh 按系统 locale (macOS AppleLocale / Linux LANG, --locale 强制覆盖) python 一行替换 description, 含别名规一 + 前缀 fallback + en 终极 fallback; 五种 locale 测试通过 (zh-Hans / ja / ar / fr-CA / xyz)
