    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("org.wezterm-ssh.gui").as_ptr()),
            )
            .unwrap();
        }
    }

    // macOS NSApplication 在 main() 之前会自己读 argv 中的
    // -AppleLanguages / -AppleLocale / -NS* 等系统级开关用于
    // localization 与各种调试。这些 argv 不是给 wezterm-gui 自己的,
    // 但 clap 会因为不识别而直接报错退出(导致 SetPreferredLanguage
    // 触发的 `open --args -AppleLanguages` relaunch 失败)。
    // 在 clap 解析前手动过滤掉 -Apple*/-NS* 开关与紧随的值。
    let opts = {
        let raw: Vec<std::ffi::OsString> = std::env::args_os().collect();
        let mut filtered: Vec<std::ffi::OsString> = Vec::with_capacity(raw.len());
        let mut iter = raw.into_iter();
        while let Some(arg) = iter.next() {
            let s = arg.to_string_lossy();
            let is_ns_flag = s.starts_with("-Apple") || s.starts_with("-NS");
            if is_ns_flag {
                // 跳过紧随的 value(可能是 array literal "(zh-Hans)" 或
                // 普通字符串 / YES / NO 等)。如果下一个 arg 也是 flag
                // (以 "-" 开头),则不消费它。
                if let Some(peek) = iter.next() {
                    let p = peek.to_string_lossy();
                    if p.starts_with('-') && p.len() > 1 {
                        // 误判:peek 是另一个 flag,放回 filtered
                        filtered.push(peek);
                    }
                    // else: 消费掉作为 NS flag 的 value
                }
                continue;
            }
            filtered.push(arg);
        }
        Opt::parse_from(filtered)
    };
