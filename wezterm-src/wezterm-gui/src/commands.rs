...
impl CommandDef {
    ...
    pub fn actions_for_palette_and_menubar(config: &ConfigHandle) -> Vec<ExpandedCommand> {
        ...
        // 31 国 Language 菜单:GUI 切换 UI 语言。click 写
        // ~/.config/wezterm/preferred_language,重启 wezterm 后菜单按新 locale 显示。
        for (locale, display_name) in crate::i18n::SUPPORTED_LOCALES {
            result.push(ExpandedCommand {
                brief: (*display_name).into(),
                doc: format!("Switch UI language to {} ({})", display_name, locale).into(),
                keys: vec![],
                action: KeyAssignment::SetPreferredLanguage((*locale).to_string()),
                menubar: &["Language"],
                icon: None,
            });
        }
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                // i18n 热重载语言切换场景:sub menu title 会从英文变成
                // 当前 locale 的译文。get_or_create_sub_menu 用 title
                // 字符串作为 lookup key,title 变了找不到旧 sub menu 就
                // 新建一个,但旧的不会被 remove(因为 mark_candidates
                // 只标记叶子 items,不标记 sub menu items),导致 main_menu
                // items 累积。下次 NSMenuTrackingSession 枚举时遇到
                // dangling 指针 → SIGSEGV(crash report 已记录)。
                // 暴力解:整个 main_menu remove_all_items,从零重建。
                existing.remove_all_items();
                mark_candidates(
                    &existing,
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&crate::i18n::t(&cmd.menubar[0]), |menu| {
                    if cmd.menubar[0] == "Window" {
                        ...
                        let services_menu = Menu::new_with_title(&crate::i18n::t("Services"));
                        services_menu.assign_as_services_menu();
                        let services_item = MenuItem::new_with(&crate::i18n::t("Services"), None, "");
                ...
                for sub_title in cmd.menubar.iter().skip(1) {
                    submenu = submenu.get_or_create_sub_menu(&crate::i18n::t(sub_title), |_menu| {});
                }
                ...
                let translated_brief = crate::i18n::t(&cmd.brief);
                let item = match submenu.get_item_with_represented_item(&represented_item) {
                    Some(existing) => {
                        existing.set_title(&translated_brief);
                ...
                // tooltip 也要翻译;t() 内部支持静态查表 + 动态模板兜底,
                // 没匹配到的回落原英文 doc。
                item.set_tool_tip(&crate::i18n::t(&cmd.doc));
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        // SetPreferredLanguage 由 actions_for_palette_and_menubar 直接 push,
        // 不经过 derive_command_from_key_assignment
        SetPreferredLanguage(_) => return None,
    })
}
