    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            SetPreferredLanguage(locale) => {
                // 不在 menu click 中重建 NSMenu(NSMenuTrackingSession 与
                // wezterm 的 spawn_into_main_thread CFRunLoopObserver 冲突,
                // 同步重建 SIGSEGV / 异步重建 stuck)。改为:写偏好文件 →
                // osascript 弹本地化对话框 → 用户点重启 → spawn detached
                // helper 重新 open .app,当前进程 exit。
                if let Err(e) = crate::i18n::set_preferred_language(locale) {
                    log::error!("set preferred language {} failed: {}", locale, e);
                } else {
                    log::info!("preferred language saved = {}; prompting restart", locale);
                    let locale_owned = locale.clone();
                    #[cfg(target_os = "macos")]
                    {
                        let display = crate::i18n::SUPPORTED_LOCALES
                            .iter()
                            .find(|(c, _)| *c == locale_owned)
                            .map(|(_, n)| (*n).to_string())
                            .unwrap_or_else(|| locale_owned.clone());
                        // 对话框文案使用**目标 locale**(用户即将切换到的语言),
                        // 这样切换到繁體中文时整个对话框是繁體中文。
                        let (title, body, btn_later, btn_restart) =
                            crate::i18n::lang_change_dialog(&locale_owned, &display);
                        // AppleScript 字符串字面量转义:\ → \\、" → \"、LF → \n
                        ...
                        let script = format!(
                            "display dialog \"{}\" buttons {{\"{}\", \"{}\"}} \
                             default button \"{}\" with title \"{}\" with icon note",
                            esc(&body),
                            esc(&btn_later),
                            esc(&btn_restart),
                            esc(&btn_restart),
                        ...
                        // 动态推导 .app bundle 路径:current_exe 是
                        // <bundle>/Contents/MacOS/wezterm-gui-real,逐级向上
                        // 找首个 .app 后缀祖先目录;不是 .app 包(开发态裸跑)
                        // 时回退到只 exit、不 relaunch。
                        let app_bundle = std::env::current_exe().ok().and_then(|p| {
                        ...
                        // POSIX shell 单引号包裹路径,内部 `'` → `'\''`。
                        let sh_quote = |s: &str| -> String {
                        ...
                        // 让 macOS 系统注入的 Window 菜单项(Fill/Center/Enter
                        // Full Screen/Move & Resize 等)随 wezterm Language 菜单
                        // 切换。流程:
                        //   1) ensure_macos_lproj:给 .app bundle 补 31 个 .lproj
                        //      与 CFBundleSupportedLocalizations(幂等)
                        //   2) defaults write <bundle_id> AppleLanguages -array
                        //      <macos_locale>:macOS 下次启动 wezterm 时按这个
                        //      AppleLanguages 渲染所有系统注入字符串
                        //   3) 用户点 Restart → lsregister -f 刷新 Launch Services
                        //      缓存 → open -n 重启
                        let macos_locale = crate::i18n::macos_locale_code(&locale_owned);
                        let bundle_id_for_defaults =
                            app_bundle.as_ref().and_then(|b| {
                                if let Err(e) = crate::i18n::ensure_macos_lproj(b) {
                                    log::warn!(
                                        "ensure_macos_lproj failed: {} (system menu items \
                                         may stay in original language)",
                                        e
                                    );
                                }
                                crate::i18n::read_bundle_id(b)
                            });
                        if let Some(bid) = bundle_id_for_defaults.as_ref() {
                            // defaults write <bid> AppleLanguages -array <macos_locale>
                            ...
                            match st {
                                Ok(s) if s.success() => log::info!(
                                    "defaults write {} AppleLanguages -array {} ok",
                                    bid,
                                    macos_locale
                                ),
                                other => log::warn!(
                                    "defaults write AppleLanguages failed: {:?}",
                                    other
                                ),
                            }
                        } else {
                            log::warn!(
                                "could not read bundle id; skipping AppleLanguages defaults"
                            );
                        }
                        let restart_label = btn_restart.clone();
                        std::thread::spawn(move || {
                            ...
                            match result {
                                Ok(out) if out.status.success() => {
                                    ...
                                    if stdout.contains(&restart_label) {
                                        log::info!("user requested restart for locale change");
                                        if let Some(bundle) = app_bundle.as_ref() {
                                            // lsregister -f:让 Launch Services 重新读
                                            // .app/Contents/Info.plist 与 .lproj 列表
                                            // killall cfprefsd:flush defaults 缓存,
                                            // 让新写的 AppleLanguages 立即可读
                                            // open --args -AppleLanguages '("zh-Hans")':
                                            // 启动时显式给 NSApplication 传 argv 形式的
                                            // localization 偏好,这是 AppKit 最高优先级的
                                            // 来源,不依赖 defaults daemon 是否已 flush
                                            let lsregister = "/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister";
                                            let bundle_quoted =
                                                sh_quote(&bundle.to_string_lossy());
                                            let lang_arg = format!(
                                                "({})",
                                                sh_quote(macos_locale)
                                            );
                                            let _ = std::process::Command::new("/bin/sh")
                                                ...
                                                .arg(format!(
                                                    "sleep 0.5; \
                                                     {ls} -f {b} >/dev/null 2>&1; \
                                                     /usr/bin/killall cfprefsd >/dev/null 2>&1; \
                                                     /usr/bin/open -n {b} \
                                                     --args -AppleLanguages {la}",
                                                    ls = lsregister,
                                                    b = bundle_quoted,
                                                    la = sh_quote(&lang_arg),
                                                ))
                                                .spawn();
                                            std::thread::sleep(
                                            ...
                                            log::warn!(
                                                "could not locate .app bundle from current_exe; \
                                                 exiting without auto-relaunch"
                                            );
                                            std::process::exit(0);
                                        }
                                    }
                                ...
                                Ok(out) => log::warn!(
                                    "osascript exit non-zero: {} stderr={}",
                                    out.status,
                                    String::from_utf8_lossy(&out.stderr)
                                ),
                                Err(e) => log::error!("spawn osascript failed: {}", e),
                            }
                        });
                    }
                    #[cfg(not(target_os = "macos"))]
                    {
                        let _ = wezterm_toast_notification::persistent_toast_notification(
                            "WezTerm Language",
                            &format!("Saved {}. Restart WezTerm to apply.", locale_owned),
                        );
                    }
                }
            }
