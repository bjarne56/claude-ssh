...
use crate::keyword::KeywordHighlightRule;
...
use wezterm_cell::{Cell, CellAttributes, Intensity, SemanticType, UnicodeVersion, Underline};
...
impl Line {
    ...
    pub fn invalidate_implicit_hyperlinks(&mut self, seqno: SequenceNo) {
        // 顺便清 keyword highlight 的 SCANNED bit:行内容变化时让下次 scan 重做。
        // 这是最少改动方案——所有 wezterm 内置的 line modification 路径都已调
        // invalidate_implicit_hyperlinks,piggy-back 上去无需新加 hook。
        self.bits &= !LineBits::SCANNED_KEYWORD_HIGHLIGHT;
    ...
    /// 与 invalidate_implicit_hyperlinks 平行:仅清掉 SCANNED_KEYWORD_HIGHLIGHT 位
    /// 让下次 scan_and_apply_keyword_highlight 重做扫描。
    ///
    /// 注意:不像 hyperlink 那样需要"卸载已写入的实际 cell 属性"——因为关键字
    /// 着色直接写 fg/bg/intensity 等基础属性,服务端发新 ANSI 时会自然覆盖,
    /// 不存在"残留 hyperlink 引用导致悬挂"问题。所以这里仅清 bit,
    /// HAS_KEYWORD_HIGHLIGHT 位作为"上次扫描的痕迹"保留无副作用。
    #[inline]
    pub fn invalidate_keyword_highlight(&mut self, _seqno: SequenceNo) {
        self.bits &= !LineBits::SCANNED_KEYWORD_HIGHLIGHT;
    }

    /// 把一组关键字高亮规则应用到当前物理行。
    ///
    /// 与 scan_and_create_hyperlinks 的区别:这里直接重写 cell 的
    /// fg/bg/intensity/italic/underline 属性,不创建 implicit hyperlink。
    /// 当 SCANNED_KEYWORD_HIGHLIGHT 位已置位且无外部失效时,本调用为 no-op,
    /// 避免重复扫描。
    pub fn scan_and_apply_keyword_highlight(&mut self, rules: &[KeywordHighlightRule]) {
        if (self.bits & LineBits::SCANNED_KEYWORD_HIGHLIGHT)
            == LineBits::SCANNED_KEYWORD_HIGHLIGHT
        {
            return;
        }
        self.bits |= LineBits::SCANNED_KEYWORD_HIGHLIGHT;
        self.bits &= !LineBits::HAS_KEYWORD_HIGHLIGHT;

        if rules.is_empty() {
            return;
        }

        let line_str = self.as_str().into_owned();
        let matches = KeywordHighlightRule::match_keywords(&line_str, rules);
        if matches.is_empty() {
            return;
        }
        ...
        // byte 索引 → cell 索引转换:跟 vecstorage::scan_and_create_hyperlinks
        // 同款做法,处理双宽字符(cell.width() == 2)。
        for (byte_idx, _grapheme) in line_str.grapheme_indices(true) {
            if cell_idx >= cells_slice.len() {
                break;
            }
            let cell = &mut cells_slice[cell_idx];
            for m in &matches {
                ...
                // override_ansi=false:仅当 cell 当前 fg 为 Default 时写入,
                // 让服务端 ANSI 颜色优先;true 时无条件覆盖。
                if let Some(fg) = rule.fg {
                    // override_ansi=false 时:仅 Default 或 TrueColor(上次 keyword 染色)可覆盖;
                    // PaletteIndex 视为服务端 ANSI,保留不动。
                    // 这样修复"line 内容变化后,旧 keyword 染色被新规则替换"的场景:
                    //   用户输入 c→ca→cat:`ca` 第二步被规则 9 染 TrueColor blue,
                    //   第三步 cat 整词应被规则 5 染紫红,需要覆盖之前的 blue。
                    let need_write = rule.override_ansi
                        || matches!(
                            attrs.foreground(),
    ...
    /// 关键字高亮的逻辑行入口,模板对偶 apply_hyperlink_rules。
    ///
    /// SecureCRT 的关键字高亮按物理行匹配,不跨行——这里也按物理行处理,
    /// 不做 hyperlink 风格的 join/split,代码更简单且与原行为更接近。
    pub fn apply_keyword_highlight_rules(
        rules: &[KeywordHighlightRule],
        logical_line: &mut [&mut Line],
    ) {
        if rules.is_empty() || logical_line.is_empty() {
            return;
        }
