...
pub struct Config {
    ...
    /// 关键字高亮规则集合(SecureCRT 风格的输出文本染色)。
    /// 每条规则匹配后会重写 cell 的颜色/字体强调属性。默认为空,
    /// 用户通过 wezterm.lua 配置开启;SecureCRT roy.ini 可经
    /// tools/secure-crt-to-wezterm-rules.py 一键迁移。
    #[dynamic(default = "default_keyword_highlight_rules")]
    pub keyword_highlight_rules: Vec<keyword::KeywordHighlightRule>,
...
/// 默认关键字高亮规则。返回空 Vec,用户在 wezterm.lua 中通过
/// `config.keyword_highlight_rules = { ... }` 显式配置;SecureCRT
/// roy.ini 的迁移产物在 ssh-ops 仓库 assets/wezterm-roy.lua,
/// 用户 `require 'wezterm-roy'` 即可启用。
pub fn default_keyword_highlight_rules() -> Vec<keyword::KeywordHighlightRule> {
    vec![]
}
...
// ssh-ops fork: 全部 cache/data/runtime 路径改用 WezTerm-SSH 命名空间, 跟原版 wezterm 隔离
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    if let Some(runtime) = dirs_next::cache_dir() {
        return Ok(runtime.join("WezTerm-SSH"));
    }

    Ok(crate::HOME_DIR.join(".local/share/WezTerm-SSH"))
}
