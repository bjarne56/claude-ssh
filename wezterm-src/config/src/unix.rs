...
impl UnixDomain {
    ...
    pub fn serve_command(&self) -> anyhow::Result<Vec<OsString>> {
        match self.serve_command.as_ref() {
            ...
            None => {
                // ssh-ops fork: 部署后 mux-server 二进制改名为 WezTerm-SSH-mux. 优先找改名后的,
                // 找不到回退原名 (兼容 cargo run 直跑).
                let exe = std::env::current_exe()?;
                let renamed_name = if cfg!(windows) {
                    "WezTerm-SSH-mux.exe"
                } else {
                    "WezTerm-SSH-mux"
                };
                ...
                let renamed = exe.with_file_name(renamed_name);
                let chosen = if renamed.exists() {
                    renamed
                } else {
                    exe.with_file_name(original_name)
                };
                Ok(vec![chosen.into_os_string(), OsString::from("--daemonize")])
            }
        }
    }
}
