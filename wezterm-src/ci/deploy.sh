...
case $OSTYPE in
  darwin*)
    # ssh-ops fork: 三个核心二进制部署时改名, 跟 Info.plist CFBundleExecutable 一致
    # (Cargo crate 名仍是 wezterm/wezterm-gui/wezterm-mux-server, 只在部署阶段 cp 改名):
    #   wezterm           → WezTerm-SSH-cli
    #   wezterm-gui       → WezTerm-SSH      (GUI binary, CFBundleExecutable)
    #   wezterm-mux-server → WezTerm-SSH-mux
    zipdir=WezTerm-SSH-macos-$TAG_NAME
    if [[ "$BUILD_REASON" == "Schedule" ]] ; then
      zipname=WezTerm-SSH-macos-nightly.zip
    else
      zipname=$zipdir.zip
    fi
    rm -rf $zipdir $zipname
    mkdir $zipdir
    cp -r assets/macos/WezTerm-SSH.app $zipdir/
    # Omit MetalANGLE for now; it's a bit laggy compared to CGL,
    # and on M1/Big Sur, CGL is implemented in terms of Metal anyway
    rm $zipdir/WezTerm-SSH.app/*.dylib
    mkdir -p $zipdir/WezTerm-SSH.app/Contents/MacOS
    mkdir -p $zipdir/WezTerm-SSH.app/Contents/Resources
    cp -r assets/shell-integration/* $zipdir/WezTerm-SSH.app/Contents/Resources
    cp -r assets/shell-completion $zipdir/WezTerm-SSH.app/Contents/Resources
    tic -xe wezterm -o $zipdir/WezTerm-SSH.app/Contents/Resources/terminfo termwiz/data/wezterm.terminfo

    for bin in wezterm wezterm-mux-server wezterm-gui strip-ansi-escapes ; do
      case "$bin" in
        wezterm-gui)        dest_name=WezTerm-SSH ;;
        wezterm)            dest_name=WezTerm-SSH-cli ;;
        wezterm-mux-server) dest_name=WezTerm-SSH-mux ;;
        *)                  dest_name=$bin ;;
      esac
      ...
      if [[ -f $TARGET_DIR/release/$bin ]] ; then
        cp $TARGET_DIR/release/$bin $zipdir/WezTerm-SSH.app/Contents/MacOS/$dest_name
      else
        ...
        lipo $TARGET_DIR/*/release/$bin -output $zipdir/WezTerm-SSH.app/Contents/MacOS/$dest_name -create
      fi
    done
    ...
    if [ -n "$MACOS_TEAM_ID" ] ; then
      ...
      /usr/bin/codesign --keychain build.keychain --force --options runtime \
        --entitlements ci/macos-entitlement.plist --deep --sign "$MACOS_TEAM_ID" $zipdir/WezTerm-SSH.app/
