...
fn delegate_to_gui(saver: UmaskSaver) -> anyhow::Result<()> {
    ...
    // ssh-ops fork: 部署后 GUI 二进制改名为 WezTerm-SSH (Cargo crate 名仍是 wezterm-gui,
    // 通过 ci/deploy.sh 与 install-local.sh 在 cp 阶段改名). 优先找改名后的, 找不到回退
    // 原名 (兼容 cargo run 直跑场景).
    ...
    let exe_dir = std::env::current_exe()?
        ...
        .to_path_buf();
    let exe = {
        let renamed = exe_dir.join(renamed_name);
        if renamed.exists() {
            renamed
        } else {
